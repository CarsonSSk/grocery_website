    ...
    $orderid = $list[$i]->orderid;
      if($orderid == $id) {
        ...
        for($j = 0; $j < count($list2); $j++) {
          ...
          $even = $even + 2;
          $odd = $odd + 2;
          $list[$i]->item[$j]->id =$newid;
          $list[$i]->item[$j]->qty = $newqty;
      }
    }
  }
...
<html lang="en">
  <head>
    ...
    <title>Edit Order | TastyGrocery.com</title>
    ...
    <div class="page-container">
        <div class="content-wrapper">
            ...
            <div class="main page-width">
              ...
              <form method = "POST" action = <?php "orders/editorder.php?id=$id" ?>>
              <div class="card-content">
                <h5 class="center-text"><b>Edit Order</b></h5>
              </div>
                    ...
                    for ($i = 0; $i < count($list); $i++) { 
                      $orderid = $list[$i]->orderid;
                        if($orderid == $id) { 
                          $list2 = $xml->order[$i]->item;
                          for($j = 0; $j < count($list2); $j++) { 
                          $itemid = $list[$i]->item[$j]->id;
                          $qty = $list[$i]->item[$j]->qty;
                          echo '<tr>';
                            echo "<td><input value= $itemid name = $codename class= 'barContentItems'</td>";
                            $codename = $codename + 1;
                            echo "<td><input value= $qty name = $codename class= 'barContentItems'</td>";
                            $codename = $codename + 1;
                          echo '</tr>';
                        }
                      }
